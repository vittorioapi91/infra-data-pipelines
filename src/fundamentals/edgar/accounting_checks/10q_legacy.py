"""
Accounting consistency checks on legacy (PEM/IMS text) scraped JSON.

Expects structure: { accession, form_type, period, company, balance_sheet, income_statement, cash_flow_statement }.
Keys are human labels (e.g. "Total Assets", "Total Net Sales And Revenues [Net Sales And Revenues From]").
Rules are loaded from src/fundamentals/edgar/rules-legacy/10q_legacy.yaml.
"""
...
def _load_rules() -> Dict[str, Any]:
    """Load 10-Q legacy rules from src/fundamentals/edgar/rules-legacy/10q_legacy.yaml."""
    mod_dir = Path(__file__).resolve().parent.parent  # edgar/
    rules_path = mod_dir / "rules-legacy" / "10q_legacy.yaml"
...
def run_all_checks(
    ...
    """Run legacy 10-Q accounting checks; return list of (check_name, passed, message, detail)."""
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: accounting_checks/10q_legacy.py <path/to/legacy.json>", file=sys.stderr)
        sys.exit(1)
    ...
    print(f"Legacy 10-Q accounting checks: {passed_count}/{len(results)} passed\n", file=sys.stderr)
